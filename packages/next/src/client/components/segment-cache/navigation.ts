...
import {
  createLinkPrefetchNavigationUnavailableError,
...
export function navigateToKnownRoute(
  ...
  if (
    ...
    // Warn when navigating via a `<Link prefetch="navigation">` to a route
    // that does not have runtime prefetching enabled (no segment exports
    // `const prefetch = 'allow-runtime'`). In that case the scheduler
    // degrades the link to the `prefetch={true}` behavior (see
    // pingRootRouteTree in scheduler.ts), so the deeper, navigation-depth
    // runtime prefetch the author asked for never happens.
    //
    // Like the `prefetch={true}` warning above, this fires at navigation
    // time rather than prefetch time, because in dev we don't prefetch on
    // viewport/hover.
    //
    // Unlike the `prefetch={true}` warning, there's no `instant = false`
    // suppression here: that export opts a route out of instant-navigation
    // validation, which isn't what this warning is about. The two fixes the
    // message documents — enable runtime prefetching on the route, or remove
    // `prefetch="navigation"` from the link — are the opt-outs.
    if (
      link !== null &&
      link.fetchStrategy === FetchStrategy.PPRNavigation &&
      (navigationSeed.routeTree.prefetchHints &
