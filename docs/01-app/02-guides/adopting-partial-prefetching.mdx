...
Partial Prefetching changes what `<Link>` downloads for a Cache Components page. Before, a `<Link>` prefetched the full rendered page including its [`use cache`](/docs/app/api-reference/directives/use-cache) data. After, it prefetches a reusable [**App Shell**](/docs/app/glossary#app-shell) per route by default. The cached data is prefetched per link only when you opt in with `prefetch={true}`.
...
When Partial Prefetching is enabled, the default cost of rendering a `<Link>` to a Cache Components page drops to "effectively free". Each visible link contributes a reusable App Shell prefetch, not a per-link page render of the cached content. The shell is downloaded once per filesystem route and reused for every link that points at that route, even if the dynamic params differ.

| `<Link>` prop                       | Before (Cache Components default)       | After Partial Prefetching                                 |
| ----------------------------------- | --------------------------------------- | --------------------------------------------------------- |
| `<Link href="/x">`                  | Prefetched the full rendered page.      | Prefetches the App Shell for `/x` only.                   |
| `<Link href="/x" prefetch>`         | Same as the default.                    | Prefetches the App Shell **and** the cached page content. |
| `<Link href="/x" prefetch={false}>` | Disabled all prefetching for this link. | Unchanged. Still disabled.                                |

The biggest change is for Cache Components pages with rich `use cache` content. Before, every `<Link>` pointing at one would prefetch the cached render. After, the cached render only ships when `prefetch={true}` is set on the link.
