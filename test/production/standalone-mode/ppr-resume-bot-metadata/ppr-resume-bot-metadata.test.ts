...
describe('ppr-resume-bot-metadata', () => {
  ...
  beforeAll(async () => {
    ...
    dynamicPostpone = (await next.readJSON('.next/server/app/dynamic.meta'))
      .postponed
    errorPostpone = (await next.readJSON('.next/server/app/error.meta'))
      .postponed

    if (
      typeof dynamicPostpone !== 'string' ||
      typeof errorPostpone !== 'string'
    ) {
      throw new Error(
        'invariant: expected the build to generate postponed state for each route'
      )
    }
  ...
  it.each([['a browser', undefined], ...BOT_USER_AGENTS])(
    ...
    async (_label, userAgent) => {
      ...
      expect($('#content').text()).toContain('dynamic content')
      expect($('title').text()).toBe('dynamic-metadata-title')

      // React must not have bailed out of the resume.
      expect(
        cliOutput.slice(before).match(/Expected the resume to render/)
      ).toBeNull()
    }
  )

  it.each(BOT_USER_AGENTS)(
    'should resume an RSC prefetch without a metadata tree mismatch for %s',
    async (_label, userAgent) => {
      const before = cliOutput.length
      ...
      const res = await fetchViaHTTP(
        ...
        withInvocationId({
          headers: {
            'x-matched-path': '/dynamic',
            'next-resume': '1',
            // Runtime prefetches build their response through
      ...
      expect(rsc).toContain('dynamic content')
      expect(
        cliOutput.slice(before).match(/Expected the resume to render/)
      ).toBeNull()
    }
  ...
  it.each(BOT_USER_AGENTS)(
    'should recover a postponed not-found without a metadata tree mismatch for %s',
    async (_label, userAgent) => {
      const before = cliOutput.length
      ...
      // The cached shell already owns the document status. The resume response
      // carries the not-found recovery payload with a successful status.
      expect(res.status).toBe(200)

      const $ = cheerio.load(await res.text())
      expect($('title').text()).toBe('error-metadata-title')
      expect(
        cliOutput.slice(before).match(/Expected the resume to render/)
      ).toBeNull()
    }
