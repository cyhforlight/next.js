...
describe('route-change-refetch - App Router', () => {
  ...
  it('refetches an open tab exactly twice when a page is added', async () => {
    ...
    // The tab never navigates after the initial load, so every further
    // request for its page in the server log is a re-fetch the server asked
    // for.
    function countServerHits() {
      return next.cliOutput.match(/ GET \/counted /g)?.length ?? 0
    }
    const baseline = countServerHits()
